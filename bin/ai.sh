...
elif grep -Fxq "$opencode_binding" "$bindings_file"; then
  doing "Set COI Opencode keybinding"
  OPENCODE_BINDING="$opencode_binding" COI_BINDING="$coi_binding" perl -0pi -e 's/\Q$ENV{OPENCODE_BINDING}\E/$ENV{COI_BINDING}/' "$bindings_file"
